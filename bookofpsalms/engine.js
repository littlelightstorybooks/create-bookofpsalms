...
LL.exportPDF = async function(pageModels, childName, goSpreadFn, spreadCount, getSpreadEls, onProgress, onDone, giverName) {
  ...
  var doc   = new jsPDF({ orientation: 'landscape', unit: 'pt', format: [595.44, 419.28] });
  var first = true;
  ...
  async function capturePage(bg) {
    ...
    if (!first) doc.addPage([595.44, 419.28], 'landscape');
    first = false;
    doc.addImage(imgData, 'JPEG', 0, 0, 595.44, 419.28);
  }
